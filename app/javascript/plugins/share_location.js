const share = () => {
	const shareButton = document.getElementById('myLocation');
	if (shareButton){
		console.log("share click");
		shareButton.addEventListener('click', (event) => {
			console.log("hello");
			navigator.geolocation.getCurrentPosition((data) => {
				let shareCoordinates = {lat: data.coords.latitude, lng: data.coords.longitude, userId: shareButton.dataset.user};
